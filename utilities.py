...
def shifted_psi_from_model(model, x, shift, normalize=True):
    """
    Devuelve psi_shift(x) = psi_gs(x - shift), evaluando directamente el modelo.

    Args:
        model      : NQS ya entrenada al GS
        x          : tensor de la malla, shape (N,1) o (N,)
        shift      : float
        normalize  : bool, si True normaliza la función de onda

    Returns:
        psi_shift : tensor complejo shape (N,)
    """
    if x.ndim == 1:
        x_eval = x[:, None]
        x_int = x
    ...
    x_shifted = x_eval - shift
    lnpsi = model(x_shifted)
    ...
    return psi
def two_soliton_superposition(model, x, x0, phi=0.0, normalize=True):
    """
    Construye una superposición de dos solitones desplazados, sin kick:
        psi(x) = psi_gs(x - x0) + exp(i phi) psi_gs(x + x0)

    Args:
        model      : NQS entrenada al GS
        x          : tensor de la malla, shape (N,1) o (N,)
        x0         : separación de cada copia respecto al origen
        phi        : fase relativa
        normalize  : si True, normaliza al final

    Returns:
        psi_total : tensor complejo shape (N,)
    """
    psi_right = shifted_psi_from_model(model, x, +x0, normalize=False)
    psi_left  = shifted_psi_from_model(model, x, -x0, normalize=False)

    phase = torch.exp(1j * torch.as_tensor(phi, dtype=psi_right.dtype, device=psi_right.device))
    psi_total = psi_right + phase * psi_left
    ...
    if normalize:
        norm = torch.trapz(torch.abs(psi_total)**2, x_int)
        psi_total = psi_total / torch.sqrt(norm)

    return psi_total
def two_soliton_with_kick(model, x, x0, k, phi=0.0, normalize=True):
    """
    Construye una superposición de dos solitones desplazados con kicks opuestos:
        psi(x) = psi_gs(x - x0) * exp(-i k x)
               + exp(i phi) * psi_gs(x + x0) * exp(+i k x)
    ...
    # Copias desplazadas, sin normalizar individualmente
    psi_right = shifted_psi_from_model(model, x_eval, +x0, normalize=False)  # centro en +x0
    psi_left  = shifted_psi_from_model(model, x_eval, -x0, normalize=False)  # centro en -x0

    phase = torch.exp(1j * torch.as_tensor(phi, dtype=psi_right.dtype, device=psi_right.device))

    kick_right = torch.exp(-1j * k * x_int)   # hacia la izquierda
    kick_left  = torch.exp(+1j * k * x_int)   # hacia la derecha

    psi_total = psi_right * kick_right + phase * psi_left * kick_left
...
def fit_model_to_target(model, x, psi_target, lr=1e-3, epochs=3000, verbose=True):
    ...
    Args:
        model       : modelo NQS a ajustar
        x           : tensor de la malla, shape (N,) o (N,1)
        psi_target  : tensor complejo shape (N,)
        lr          : learning rate del optimizador
        epochs      : número de épocas
        verbose     : si True muestra barra de progreso y pérdida
...
def two_soliton_analytical(x, N1, N2, x0, k1, k2, phi=0.0, g=-1.0):
    """
    Two-soliton initial state built from analytical profiles.
    Soliton 1: mass N1, centered at +x0, kick -k1 (moving left)
    Soliton 2: mass N2, centered at -x0, kick +k2 (moving right)
    """
    import benchmarks as bm
    x_np = x.detach().cpu().numpy().squeeze() \
           if hasattr(x, 'detach') else np.asarray(x)
    
    psi1 = bm.bright_soliton_wavefunction(x_np, N1, +x0, -k1, g)
    psi2 = bm.bright_soliton_wavefunction(x_np, N2, -x0, +k2, g)
    
    phase = np.exp(1j * phi)
    psi_total = psi1 + phase * psi2
    
    # Convert to torch
    psi_total = torch.tensor(psi_total, dtype=torch.complex128)
    return psi_total